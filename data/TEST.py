...
class TestData(Dataset):
    ...
    '''
    def __init__(self,csv_path,w,h,transform=None):
        """
            
        """
        self.landmark = pd.read_csv(csv_path,error_bad_lines=False)
        self.w = w
        self.h = h
    ...
    def __getitem__(self,idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        img_path = self.landmark.iloc[idx,0]
        #img_path = os.path.join("/ext_disk3/data_liuyang/Mask_dataset",img_path)
...
if __name__ == "__main__":
    dt = TestData(csv_path="label.csv",
                              w=112,
                              h=112)
